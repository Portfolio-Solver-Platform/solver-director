...
def test_create_project(client_with_db, auth):
    ...
    with patch("src.routers.api.projects.start_project_services") as mock_start, patch(
        ...
        # Verify start_project_services was called with project_id (UUID string) and user_id
        mock_start.assert_called_once()
        call_args = mock_start.call_args[0]
        assert isinstance(call_args[0], str)  # project_id as UUID string
...
def test_create_project_empty_configuration(client_with_db, auth):
    ...
    assert response.status_code == 422
    assert "at least 1" in str(response.json()).lower()


def test_create_project_invalid_problem_group(client_with_db, auth):
    """Test creating project with invalid problemGroup ID returns 422"""
    token = auth.issue_token(MockToken(scopes=["projects:write"]))
    invalid_config = {
        "name": "Invalid Project",
        "configuration": [
            {
                "problemGroup": 0,  # Invalid: must be > 0
                "solvers": [1],
    ...
    assert response.status_code == 422
    assert "greater than 0" in str(response.json()).lower()


def test_create_project_empty_solvers(client_with_db, auth):
    """Test creating project with empty solvers list returns 422"""
    token = auth.issue_token(MockToken(scopes=["projects:write"]))
    invalid_config = {
        "name": "Invalid Project",
        "configuration": [
...
def test_get_project_config(client_with_db, auth):
    ...
    with patch("src.routers.api.projects.start_project_services"), patch(
        ...
        # Verify configuration matches full ProjectConfiguration (includes name + configuration)
        assert config["name"] == VALID_CONFIG["name"]
        assert config["configuration"] == VALID_CONFIG["configuration"]
        assert len(config["configuration"]) == 1
        assert config["configuration"][0]["problemGroup"] == 1
        assert config["configuration"][0]["solvers"] == [1, 2]
        assert len(config["configuration"][0]["problems"]) == 2
